...
new_actor.save
# puts "There are #{Actor.all.count} actor"
...
# puts "There are #{Studio.all.count} studio"
...
# puts "There are #{Movie.all.count} movie"

Role.destroy_all

batmanbegins = Movie.find_by({"title" => "Batman Begins"})
thedarknight = Movie.find_by({"title" => "The Dark Knight"})
thedarknightrises = Movie.find_by({"title" => "The Dark Knight Rises"})
...
hatthaway = Actor.find_by({"name" => "Anne Hathaway"})
...
new_role["movie_id"] = thedarknightrises["id"]
new_role["actor_id"] = hatthaway["id"]
new_role.save
...
for movie in movies_list
    ...
    puts "#{movie["title"]} #{movie["year"]} #{movie["rated"]} #{studio}"
end
